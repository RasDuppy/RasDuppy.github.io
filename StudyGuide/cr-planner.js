...
function spTrackResult(q, correct, game, earnings){
  if(!q) return;
  // Record in all-time results log
  studyPlan.results.push({
  ...
  if(typeof earnings === 'number' && earnings > 0){
    studyPlan.earnings[game] = (studyPlan.earnings[game] || 0) + earnings;
  }
  _spSave();
}
...
function renderPlannerGames(){
  ...
  const totalQ = hasSel ? allQ().filter(q=>sel.some(s=>s.chKey===q.chapter&&s.topicKey===q.topic)).length : 0;
  ...
  sumDiv.innerHTML = `<span style="font-family:var(--mono);font-size:.65rem;color:var(--muted);">Active topics:</span>${chips}`;
...
function launcherGo(){
  ...
  activeChapter = sel[0].chKey;
  activeTopic   = sel.length===1 ? sel[0].topicKey : null;
...
function launchGame(game){
  const sel = studyPlan.selections;
  if(!sel.length){
    // No selections — go to planner topics tab
    launcherGame = game;
    showPlanner('topics');
    return;
  }
  ...
  activeChapter = valid[0].chKey;
  activeTopic = valid.length === 1 ? valid[0].topicKey : null;
...
/* ── Shared game dispatch ── */
function _dispatchGame(game, sel){
  const firstChKey = sel && sel[0] ? sel[0].chKey : null;
  switch(game){
    case 'jeopardy':     startJeopardyMulti(sel); break;
    case 'practice':     startPracticeMulti(sel); break;
    case 'millionaire':  startMillionaire(firstChKey); break;
    case 'flashcards':   startFlashcards(firstChKey); break;
    case 'crossword':    startCrossword(firstChKey); break;
    case 'wordsearch':   startWordSearch(firstChKey); break;
    case 'wof':          startWof(firstChKey); break;
    case 'wordle':       startWordle(); break;
    case 'connections':  startConnections(); break;
  }
}
