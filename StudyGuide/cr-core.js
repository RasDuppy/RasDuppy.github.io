...
function showHub(){
  ...
  if(total === 0 && vCount === 0){
    n.style.color = '';
    n.innerHTML = '⚠ No data loaded. Go to <b style="color:var(--gold)">✎ Manage Questions</b> → import <b>questions.csv</b> and/or <b>vocab.csv</b>.';
  } else {
    n.style.color = 'var(--green)';
    // Count by type — 'vocab' is the generic bucket; 'ion' and 'element' are optional chem types
    const typeCounts = {};
    vocabBank.forEach(v => { const t = v.type || 'vocab'; typeCounts[t] = (typeCounts[t]||0)+1; });
    const _pts = [];
    if(total)   _pts.push(total + (total!==1?' questions':' question'));
    Object.entries(typeCounts).forEach(([t, c]) => _pts.push(c + ' ' + t));
...
// ── RANDOMIZER REGISTRY ──────────────────────────────────────────────────────
// The built-in randomizers below are CHEMISTRY-SPECIFIC (gas laws, titrations,
// stoichiometry, pH). They are fully optional — teachers for other subjects
// simply leave the `randomizer_type` column blank in their questions.csv.
//
// To add randomizers for another subject (e.g. physics, economics):
//   1. Add a new key to RANDOMIZER_REGISTRY below.
//   2. The key must exactly match the `randomizer_type` value in the CSV.
//   3. The function must return an object overriding: q, given_keys,
//      given_vals_display, answer_raw, answer_display, answer_sig_figs,
//      tolerance, unit, type, hint, sol_1...sol_4.
//
// generateVariant(templateQ) applies the registry and returns null if the
// randomizer_type is not registered — so unrecognised types fail gracefully.
// ─────────────────────────────────────────────────────────────────────────────
