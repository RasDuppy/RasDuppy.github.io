...
let customBank = [];
// vocabBank: unified store — type=vocab is generic; type=ion and type=element are
// chemistry-specific extended types. Any subject can define its own types —
// they will be stored here and counted by type in the hub notice.
let vocabBank  = [];
// NOTE on localStorage keys: 'chemq_custom' and 'chemq_vocab' are the legacy keys
// preserved for backwards compatibility. A future multi-subject update will namespace
// these per subject (e.g. 'srq_subject_apbio_questions'). For now, importing a new
// subject CSV replaces the existing bank — students should import one subject at a time.
...
function exportCSV(){
  _downloadText(questionsToCSV(allQ()), 'studyreview-questions.csv');
}
function exportVocabCSV(){
  _downloadText(vocabToCSV(vocabBank), 'studyreview-vocab.csv');
}
...
function importVocabCSV(file){
  ...
  reader.onload = e => {
    try {
      ...
      _saveVocab();
      rebuildChapters();
      // Count by type for the success message
      const typeCounts = {};
      imported.forEach(v => { const t = v.type||'vocab'; typeCounts[t] = (typeCounts[t]||0)+1; });
...
let activeChapter = null; // legacy — kept for compatibility
let activeTopic   = null; // legacy
// NEW: multi-selection. Array of {chKey, topicKey} objects.
...
function boardQ() {
  ...
  // Legacy fallback
  return active.filter(q =>
    (!activeChapter || q.chapter === activeChapter) &&
