...
function startWordle(){
  ...
  // Default to ions if available, vocab otherwise
  const mode = vocabBank.some(v=>v.type==='ion' && v.enabled!==false && v.wordle_eligible!==false) ? 'ions' : 'vocab';
  _wdlSetModeUI(mode);
  wdlNewGame();
}
...
function _wdlSetModeUI(mode){
  wdlState = wdlState || {};
  wdlState.mode = mode;
  document.getElementById('wdl-mode-ions').classList.toggle('active', mode==='ions');
  document.getElementById('wdl-mode-vocab').classList.toggle('active', mode==='vocab');
...
function _wdlPickWord(mode){
  ...
  if(mode === 'ions'){
    ...
    // vocab mode — respect vocabSelections chapters chosen in planner
    const vocabPool = (typeof _getActiveVocabPool === 'function')
      ? _getActiveVocabPool('wordle')
      : vocabBank.filter(v => v.enabled !== false && v.wordle_eligible !== false && !v.is_multiword);
    pool = vocabPool
      .filter(v => v.type === 'vocab')
      .map(v => ({
        word:        v.word,
        displayWord: v.word.toLowerCase(),
        clue:        v.definition,
  ...
  if(!picked){
    document.getElementById('wdl-body').innerHTML = `
      <div style="color:var(--muted);font-family:var(--mono);font-size:.8rem;text-align:center;padding:40px;line-height:1.9;">
        ${mode==='ions'
          ? '⚗ No ions loaded yet.<br>Import your <b>ions.csv</b> via ✎ Manage Questions → ⬆ Ions CSV'
          : '📖 No vocab words found for the selected chapters.<br>Import a vocab.csv or select a chapter with vocab.'}
      </div>`;
    return;
  }
