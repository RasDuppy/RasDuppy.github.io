...
class QuestionEngine {
  ...
  constructor(pool, options = {}) {
    this.pool        = pool;                      // full question pool
  ...
  next(forcedTemplate = null) {
    const tmpl = forcedTemplate || weightedPick(this.pool) || this.pool[Math.floor(Math.random() * this.pool.length)];
  ...
  render(containerEl, renderOptions = {}) {
    ...
    // ── Image URL ─────────────────────────────────────────────
    // Renders a plain image above the question — works for any subject.
    // Teachers put a direct image URL in the image_url CSV column.
    // Supports: photos, diagrams, maps, charts, artwork, etc.
    if (q.image_url) {
      ...
      img.src = q.image_url;
      img.alt = q.q || 'Question image';
      img.style.cssText = 'max-width:100%;max-height:320px;border-radius:6px;object-fit:contain;';
    ...
    if (this.template?.randomizer_type) {
      ...
      info.textContent = `⚙ This question type generates infinite variants. Randomizer: ${this.template.randomizer_type}`;
